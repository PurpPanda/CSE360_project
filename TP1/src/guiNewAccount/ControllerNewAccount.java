...
public class ControllerNewAccount {
	...
	*/
	...
	protected static void doCreateUser() {
		...
		// -------------------- TP1 Input Validation --------------------

		// 1) Username (Email)
		String emailError = InputValidator.validateEmail(ViewNewAccount.emailAddress);
		if (!emailError.isEmpty()) {
		    ViewNewAccount.alertUsernamePasswordError.setContentText(emailError);
		    ViewNewAccount.alertUsernamePasswordError.showAndWait();
		    return;
		}

		// 2) Password (length + complexity)
		String pwError = InputValidator.validatePassword(password);
		if (!pwError.isEmpty()) {
		...
		// 2b) Confirm password matches
		String matchError = InputValidator.validatePasswordMatch(
		        ViewNewAccount.text_Password1.getText(),
		...
		if (!matchError.isEmpty()) {
		    ...
		    ViewNewAccount.alertUsernamePasswordError.setContentText(matchError);
		    ViewNewAccount.alertUsernamePasswordError.showAndWait();
		    return;
		}

		// NOTE: This NewAccount page does NOT collect phone/name fields,
		// those are handled on the UserUpdate page.
		// --------------------------------------------------------------
