...
public class ViewUserUpdate {
	...
	private static Label label_CurrentPhoneNumber = new Label("<none>");
	...
	private ViewUserUpdate() {
     ...
     // First Name
        ...
        button_UpdateFirstName.setOnAction((_) -> {
            ...
            String err = InputValidator.validateName(result.get(), true, "First name");
            if (!err.isEmpty()) {
                alertInputError.setTitle("Invalid First Name");
                alertInputError.setHeaderText("Name Error");
                alertInputError.setContentText(err);
        ...
        button_UpdateMiddleName.setOnAction((_) -> {
            ...
            String err = InputValidator.validateName(result.get(), false, "Middle name");
            if (!err.isEmpty()) {
                alertInputError.setTitle("Invalid Middle Name");
                alertInputError.setHeaderText("Name Error");
                alertInputError.setContentText(err);
                alertInputError.showAndWait();
                return;
        ...
        button_UpdateLastName.setOnAction((_) -> {
            ...
            String err = InputValidator.validateName(result.get(), true, "Last name");
            if (!err.isEmpty()) {
                alertInputError.setTitle("Invalid Last Name");
                alertInputError.setHeaderText("Name Error");
                alertInputError.setContentText(err);
                alertInputError.showAndWait();
                return;
        ...
        button_UpdatePreferredFirstName.setOnAction((_) -> {
            ...
            String err = InputValidator.validateName(result.get(), false, "Preferred name");
            if (!err.isEmpty()) {
                alertInputError.setTitle("Invalid Preferred Name");
                alertInputError.setHeaderText("Name Error");
                alertInputError.setContentText(err);
                alertInputError.showAndWait();
                return;
        ...
        button_UpdateEmailAddress.setOnAction((_) -> {
            ...
            String err = InputValidator.validateEmail(result.get());
            if (!err.isEmpty()) {
                alertInputError.setTitle("Invalid Email Address");
                alertInputError.setHeaderText("Email Error");
                alertInputError.setContentText(err);
                alertInputError.showAndWait();
                return;
            }
        ...
        // Phone Number 
        setupLabelUI(label_PhoneNumber, "Arial", 18, 190, Pos.BASELINE_RIGHT, 5, 425);
        setupLabelUI(label_CurrentPhoneNumber, "Arial", 18, 260, Pos.BASELINE_LEFT, 200, 425);
        setupButtonUI(button_UpdatePhoneNumber, "Dialog", 18, 275, Pos.CENTER, 500, 418);
        ...
        button_UpdatePhoneNumber.setOnAction((_) -> {
            ...
            String err = InputValidator.validatePhoneNumber(phone);
            if (!err.isEmpty()) {
                alertInputError.setTitle("Invalid Phone Number");
                alertInputError.setHeaderText("Phone Error");
                alertInputError.setContentText(err);
                alertInputError.showAndWait();
                return;
            }

            label_CurrentPhoneNumber.setText(phone);
        });


        // Proceed button: block leaving page unless phone is valid (required)
        ...
        button_ProceedToUserHomePage.setOnAction((_) -> {
            ...
            String phone = label_CurrentPhoneNumber.getText();
            if (phone == null || phone.equals("<none>")) phone = "";

            String err = InputValidator.validatePhoneNumber(phone);
            if (!err.isEmpty()) {
                alertInputError.setTitle("Phone Number Required");
                alertInputError.setHeaderText("Phone Error");
                alertInputError.setContentText(err);
                alertInputError.showAndWait();
                return;
